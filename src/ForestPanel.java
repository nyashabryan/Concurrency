	...
	public void paintComponent(Graphics g) {
		...
		for(int layer = 0; layer <= 10; layer++) {
			for(int t = 0; t < forest.length; t++){
				...
				if(forest[rt].getExtent() >= minh && forest[rt].getExtent() < maxh) { // only render trees in current band
					...
					g.fillRect(forest[rt].getY() - (int) forest[rt].getExtent(), forest[rt].getX() - (int) forest[rt].getExtent(),
						   2*(int) forest[rt].getExtent()+1,2*(int) forest[rt].getExtent()+1);
				}
